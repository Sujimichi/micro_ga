...
Args can be passed as a hash.  The following shows valid keys for args and thier default values if not given;
  ...
  :generations => 400,
  :fitness =>  Proc.new{|genome| puts(genome.inspect); genome.inject{|i,j| i+j} }
